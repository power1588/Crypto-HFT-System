...
use crypto_hft::core::events::{NewOrder, OrderSide, Position, RiskViolation, TimeInForce};
...
async fn test_risk_engine_basic_operations() {
    ...
    risk_engine
        .update_balance("USDT", Size::from_str("10000.0").unwrap())
        .await;
    ...
    risk_engine
        .update_position("BTCUSDT", position.clone())
        .await;
...
async fn test_balance_rule_type_conversions() {
    ...
    risk_engine
        .update_balance("BTC", Size::from_str("1.0").unwrap())
        .await;
