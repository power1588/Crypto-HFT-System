...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Default to testnet/dry-run mode
    let dry_run = !args.contains(&"--live".to_string());

    // Determine strategy type from command line
    let strategy_type = if args.contains(&"--arbitrage".to_string()) {
        "arbitrage"
    } else if args.contains(&"--market-making".to_string()) {
        "market-making"
    } else {
    ...
    info!("Starting {} System", strategy_type);
    info!("Mode: {}", if dry_run { "Dry-Run" } else { "Live" });
    info!("Symbol: {}", symbol);

    if dry_run {
        // Run in dry-run mode with Binance WebSocket for market data
        run_dry_run_strategy(&symbol, strategy_type).await
    } else {
        // Live mode requires API keys
        eprintln!("⚠️  Live trading is not yet implemented.");
        eprintln!("Please use --dry-run mode or run the binance_dry_run binary.");
        Ok(())
    }
}

/// Run strategy in dry-run mode
async fn run_dry_run_strategy(
    symbol: &str,
    strategy_type: &str,
) -> Result<(), Box<dyn std::error::Error>> {
    info!("Initializing {} Strategy in Dry-Run Mode", strategy_type);
    ...
    binance_ws.connect(&[symbol]).await.map_err(|e| {
    ...
    let order_book = Arc::new(tokio::sync::RwLock::new(OrderBook::new(symbol.to_string())));
    ...
    println!("\n╔════════════════════════════════════════════════════════════╗");
    println!("║ 🎯 {} Market Making (Dry-Run Mode)                   ║", symbol);
                ...
                Ok(event) => {
                    ...
                    if should_generate {
                        ...
                        let market_state = {
                            ...
                            let mut state = MarketState::new(symbol.to_string());
                            ...
                            let snapshot = crypto_hft::core::events::OrderBookSnapshot::new(
                                symbol.to_string(),
                                "binance".to_string(),
                                bids,
                                asks,
