...
impl SimpleArbitrageStrategyImpl {
    ...
    pub fn update_position(&mut self, exchange: &str, symbol: &str, quantity_change: Size) {
        ...
        let current_position = exchange_positions
            ...
            .or_insert_with(|| Size::new(rust_decimal::Decimal::ZERO));
        ...
        *current_position = *current_position + quantity_change;
        ...
        // Note: The entry is already modified in place, no need to insert again
    }
    ...
    pub fn can_trade(&self, exchange: &str, symbol: &str, quantity: Size) -> bool {
        ...
        let available = if current_position > Size::new(rust_decimal::Decimal::ZERO) {
...
impl Strategy for SimpleArbitrageStrategyImpl {
    fn generate_signal(&mut self, market_state: &MarketState) -> Option<Signal> {
        ...
        // Calculate spread (Price - Price returns Price, not Option<Price>)
        let spread = {
            let diff = ask_price - bid_price;
            if diff > Price::new(rust_decimal::Decimal::ZERO) {
                diff
            } else {
        ...
        // spread * trade_quantity returns Decimal, wrap in Price for expected_profit
        let profit = spread * trade_quantity;
        Some(Signal::Arbitrage {
            ...
            expected_profit: Price::new(profit),
        })
    }
}
